...
sp_df_list <- list.files(path = "CSV_Files", pattern = "clean_df.csv")
...
#load data frames 
QUAC_wK_PCA_df <- read.csv("CSV_Files/QUAC_allpop_clean_df.csv")
ZAIN_rebinned_PCA_df <- read.csv("CSV_Files/ZAIN_rebinned_allpop_clean_df.csv")
...
for(sp in 1:length(scenario_list)){
  ...
  #load data frame 
  sp_df <- read.csv(paste0("CSV_Files/", sp_df_list[[1]]))
  
  #separate wild individuals 
  sp_wild_genind <- repool(seppop(sp_genind_temp)[pop_list[[1+5]]])
...
for(sp in 1:length(scenario_list)){
  ...
  sp_df <- read.csv(paste0("CSV_Files/", sp_df_list[[sp]]))
...
#read in data frame 
QUAC_wk_df <- read.csv("CSV_Files/QUAC_allpop_clean_df.csv")
