...
QUAC_woK_list <- list.files(path = "Structure_Files/QUAC_wild_woK", pattern = ".csv")
QUAC_garden_wild_wk_list <- list.files(path = "Structure_Files/QUAC_allpop_wK", pattern = ".csv")
QUAC_pop_list <- list()
QUAC_woK_pop_list <- list()
...
for(k in 1:length(QUAC_garden_wild_wk_list)){
  ...
  QUAC_k <- read.csv(paste0("Structure_Files/QUAC_allpop_woK/",QUAC_garden_wild_wk_list[[k]]))
  
  QUAC_k_ready <- QUAC_k[,-c(1:2)]
  
  colors <- viridis::viridis(k+1)
  
  pdf(paste0("../Analyses/Results/Clustering/Structure/QUAC_garden_wild_woK_STR_k",k+1,".pdf"), width = 20, height = 10)
  # Graphing parameters
  par(mar=c(7,2,10,1)+0.1, mgp = c(3,1,1))
