...
<ul><li><b>Structure_Files</b>are the files used to run the program STRUCTURE (Pritchard et al. 2000) and also the results files from this program. The files deemed "_str_READY.txt" are the files STRUCTURE was actually run on.  The "_str_READY.txt" file is a text file with a similar layout to the CSV files described above, just with individual names in each row and a population name assigned as numerals. There are no column headers in this text file.</li></ul>
<ul><li><b>Geneclass files</b> are the files used to run the Geneclass 2 software (Piry et al. 2004) for assignment into source populations. The input and output files are both text files that are differentiated from one another with the label "input" and "output" in the name.</li></ul>
