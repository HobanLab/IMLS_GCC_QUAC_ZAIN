...
<ul><li><b>Geneclass files</b> are the files used to run the Geneclass 2 software (Piry et al. 2004) for assignment into source populations. The input files for Geneclass are defined using "input" and are in the Genepop format as described above, but with numeric coding for alleles in a two digit format. Additionally, in this folder, the Genalex files to generate the each input Genepop file is included. Genalex (Peakall and Smouse, 2006) was used to convert these files into the input Genepop files for the Geneclass software. The "wild" genepop files are used as the reference populations in the Geneclass software and the "garden" genepop files are used as the samples to be assigned in the software.</li></ul>
...
<ul><li>Any file named as "clean" indicates that these files were cleaned in "2_clonecheck_md_relatedness.R" script, which removes individuals with too much missing data (over 25%) and clones. “Clean” data files are used for all other genetic analyses in the study.</li></ul>
<ul><li>“ZAIN_og” files are the original data files used in analysis, with the originally scored genetic data. A common problem when scoring microsatellites is some degree of subjectivity in calling an allele with a decimal number into an integer-based bin, and in determining the location of peaks. When microsatellites are called by different people, sometimes scoring differences result. Our microsatellite data were scored by different researchers over the course of ~10 years, so we performed an analysis to standardized microsatellite scores between researcher, and these data files are named "ZAIN_rebinned" files. We also performed all analyses on "ZAIN_og" and "ZAIN_rebinned" data to determine the impact of scoring on our results. </li></ul>
